...
function emitThreadInfoEvent(ctx: Loose, globalCallback: Loose, payload: Loose) {
...
function createParseDelta(deps: Loose) {
      ...
      const resolveAttachmentUrl = (i: number) => {
        if (!delta.attachments || i === delta.attachments.length || getType(delta.attachments) !== "Array") {
          ...
          if (attachment && attachment.mercury && attachment.mercury.attach_type === "photo") {
            api.resolvePhotoUrl(attachment.fbid, (err: Loose, url: string) => {
          ...
          if (d.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
            ...
            try {
              ...
              callbackToReturn = {
                ...
                attachments: (msg.attachments || []).map((att: Loose) => {
                  try {
                ...
                participantIDs: (msg.participants || []).map((e: Loose) => e.toString())
              };
            if (d.deltaMessageReply.repliedToMessage) {
              try {
                ...
                if (msgMetadata && msgMetadata.threadKey) {
                  callbackToReturn.messageReply = {
                    ...
                    messageID: msgMetadata.messageId || "",
                    senderID: (msgMetadata.actorFbId || "").toString(),
                    attachments: (repliedTo.attachments || []).map((att: Loose) => {
                      let mercury: Loose;
                      try {
                    ...
                    mentions: rmentions,
                    timestamp: parseInt(msgMetadata.timestamp || 0),
                    participantIDs: (repliedTo.participants || []).map((e: Loose) => e.toString())
                  };
                }
              } catch (err: Loose) {
              ...
              return defaultFuncs.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, {
              ...
              }).then(parseAndCheckLogin(ctx, defaultFuncs)).then((resData: Loose) => {
                if (resData[resData.length - 1].error_results > 0) throw resData[0].o0.errors;
                if (resData[resData.length - 1].successful_results === 0) throw { error: "forcedFetch: there was no successful_results", res: resData };
                const fetchData = resData[0].o0.data.message;
                const mobj: Record<string, Loose> = {};
                ...
                callbackToReturn.messageReply = {
                  ...
                  attachments: fetchData.message.blob_attachment.map((att: Loose) =>
                    _formatAttachment({ blob_attachment: att }, undefined)
                  ),
    ...
    switch (delta.class) {
      ...
      case "ForcedFetch": {
        ...
        if (mid && tid) {
          ...
          defaultFuncs.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, form).then(parseAndCheckLogin(ctx, defaultFuncs)).then((resData: Loose) => {
            if (resData[resData.length - 1].error_results > 0) throw resData[0].o0.errors;
            if (resData[resData.length - 1].successful_results === 0) throw { error: "forcedFetch: there was no successful_results", res: resData };
            const fetchData = resData[0].o0.data.message;
